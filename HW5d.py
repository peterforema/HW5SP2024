...
def RSquared(x, y, coeff):
    ...
    :param coeff: The coefficients of the polynomial fit as a tuple or list.
...
def PlotLeastSquares(x, y, coeff, showpoints=True, npoints=500):
    ...
    pyplot.gca().tick_params(axis='both', top=True, right=True, direction='in', grid_linewidth=1,
                             grid_linestyle='dashed', grid_alpha=0.5)
    pyplot.show()
    return xvals, yvals
...
def LeastSquaresFit(x, y, power=1):
    ...
    coeff, cov = curve_fit(Poly, x, y, a)
    return coeff
...
def main():
    ...
    pyplot.plot(linx, liny, linewidth=2, linestyle='dashed', color='black',
                label=r'Linear fit ($R^2={:0.3f}$)'.format(RSqLin))
    pyplot.plot(cubx, cuby, linewidth=2, linestyle='dotted', color='black',
                label='Cubic fit ($R^2={:0.3f}$)'.format(RSqCub))
    pyplot.plot(x, y, linestyle='none', marker='o', markersize=10, markerfacecolor='white', markeredgecolor='black',
                label='Data')
    pyplot.xlabel('X values')
    pyplot.ylabel('Y values')
    pyplot.legend()
